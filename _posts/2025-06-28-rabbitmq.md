...
- Synchronous communication: The client sends a request and waits for the server to respond right away. This requires the server to be available at the exact same time. That tight coupling can lead to:
...
In RabbitMQ, queues (FIFO datastructure) are the mailboxes that stores messages until they are consumed. The process, either server or a job, that sends the message to the queue is called Producer and the process that gets the messages from the queue is called Consumer. Let's walk through a bird's eye view of what happens when a message is sent. It's the easiest way to understand the concepts and the terminology:
1. Producer sends a message to an exchange. Queues are bound to exchanges. Think of the exchange as a smart traffic controller. It's job is to accept messages from producers and then decides which queue(s) to send the message to
2. The exchange doesn’t hold any messages but routes the message to one or more queues bound to it
3. Consumers pull (or are pushed) messages from the queues (and not an exchange)

The server/software (RabbitMQ as an example) that manages exchanges, queues and message delivery is called the Broker. 
...
> Note: Once the TCP connection is established, producers and consumers open channels within that connection. These channels are the primary units for communication in RabbitMQ. Think of channels as lightweight, virtual connections inside a single TCP connection. Instead of opening a new TCP connection for every producer or consumer, multiple channels can be created to send and receive messages efficiently. This approach reduces resource usage and enables concurrent operations over the same underlying connection.
...
2. **Fanout Exchange**
Broadcasts a message to all queues bound to it, ignoring the routing key. This exchange is used when we want every consumer to get a copy of every message. For example:
...
Once a consumer has succesfully processed a message, it sends an `ACK` back to the broker. If the broker doesn't recieve an `ACK`, it assumes the message wasn't processed and nd will redeliver it - either to the same consumer (if still connected) or to another one.
...
This is the recommended mode for production systems. Here, the consumer explicitly sends an ACK only after successfully processing the message. If the consumer disconnects or crashes without an acknowledgment, RabbitMQ detects the lost connection and automatically re-queues the message for redelivery. RabbitMQ doesn’t guarantee ordering when re-queuing, that is, the message usually goes to the back of the queue.
...
7. Declare the queues as durable.
8. Add mechanisms to mitigate requeue storms. 
